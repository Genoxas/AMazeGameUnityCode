...
public class PlayerLobby : NetworkLobbyPlayer
{
    ...
    //When someone joins the lobby, create a playerCanvas prefab for that player. 
    ...
    //When a player leaves the lobby, destroy the canvas that represented that player
    ...
    //Sets the ready state for each player in the lobby
    ...
    {
        var lobby = NetworkManager.singleton as Test;
        if (lobby == null)
        {
            return slot * 200;
        }
